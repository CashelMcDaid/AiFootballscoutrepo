...
function buildScoredPlayers(roleFilter, defensiveWeight) {
  ...
  const defFactor = Math.pow(rawDef, 2.2);
  const attFactor = Math.pow(rawAtt, 2.2);
  ...
  for (const p of pool) {
    ...
    const isDefender = ["CB", "DM", "FB", "GK"].includes(p.detailedPosition);
    const styleSwing = isDefender ? lean * 28 : (-lean) * 28;
    const roleBias = isDefender ? defBlend * 10 : attBlend * 10;
    const overallScore = Math.max(1, Math.min(100, Math.round(weightedCore * 0.72 + baseScore * 18 + styleSwing + roleBias)));
...
function ensureFooter() {
  ...
  footer.innerHTML = `<div class="footer-contact footer-contact-only"><span>Email: <a href="mailto:l00170935@atu.ie">l00170935@atu.ie</a></span><span>Phone: 086 417 2096</span></div>`;
...
async function loadSuggestions() {
  ...
  weeklySuggestionBox.innerHTML = `<h4>Monthly scout recommendations · 2026</h4>`;
  if (!currentSuggestions.length) {
    weeklySuggestionBox.innerHTML += `<p>No 2026 monthly recommendations loaded.</p>`;
  } else {
    weeklySuggestionBox.innerHTML += `<div class="inline-suggestion-grid">${currentSuggestions.map((item) => `
      <div class="suggestion-item compact-suggestion">
        ...
        <div><strong>Why this month:</strong> ${esc(item.because)}</div>
        <div><strong>Model drivers:</strong> ${esc(item.generatedFrom)}</div>
      </div>`).join("")}</div>`;
  }

  scoutSuggestionList.innerHTML = currentSuggestions.length ? `<div class="suggestion-list-inner">${currentSuggestions.map((item) => `
...
async function deleteAdminPlayer() {
  ...
  const id = adminPlayerSelect.value;
  if (!id) return;
  const confirmed = window.confirm("Delete this player from the local dataset?");
  if (!confirmed) return;
  const res = await fetch(`/api/admin/player/${encodeURIComponent(id)}`, {
    method: "DELETE",
    headers: authHeaders()
  ...
  const data = await res.json();
  if (!res.ok || !data.ok) {
    adminEditStatus.textContent = data.message || "Could not delete player.";
    return;
  }
  adminEditStatus.textContent = `${data.player} deleted.`;
  await loadLeaguesAndData();
  await loadSuggestions();
  await loadAdminSummary();
}
...
if (adminDeletePlayerBtn) adminDeletePlayerBtn.addEventListener("click", () => {
  deleteAdminPlayer().catch((err) => {
    console.error("Admin delete error:", err);
    adminEditStatus.textContent = "Could not delete player.";
  });
});
